...
/// Builds a slice from a C pointer and length, empty and safe when the length is
/// zero so a null pointer is never dereferenced.
unsafe fn read_slice<'a>(ptr: *const f64, n: usize) -> Option<&'a [f64]> {
    if n == 0 {
        Some(&[])
    } else if ptr.is_null() {
        None
