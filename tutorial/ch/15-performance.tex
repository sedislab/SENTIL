...
\begin{notebox}
Everything in this chapter ran on one machine, the same AMD EPYC~7763, and every timing is the mean over repeated runs, thirty at the smaller sizes and a million individual updates for the streaming figures, with the tail reported as the 99th-percentile. The deterministic tier reruns in CI on every commit, and a tolerance drift fails the build so we can be reasonably sure that no change has caused a regression in performance.
\end{notebox}