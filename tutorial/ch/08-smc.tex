...
A mean well above zero with a modest spread is a comfortable margin but a mean near zero with a wide spread is a property that's passing by a hair. The \code{variance} is the population variance (divide by $N$), and \code{std\_dev} is its root.
