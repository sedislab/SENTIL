...
The online monitor evaluates on the samples as they arrive and reads whether a formula is probabilistic from its `P` prefix, so the `algorithm` and `semantics` fields, with the statistical settings, configure the offline `sentil_record_analyzer` rather than the online monitor.
