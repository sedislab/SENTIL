...
* A lifecycle control node that synthesizes control from a spec and actuates it on
  ROS 2 topics, with three modes: receding_horizon online control under a deadline,
  open_loop trajectory synthesis, and a safety_filter control-barrier shield over a
  nominal command. Ships the Control message, the sentil_control launch file, and the
  control_loop example.