...
public final class Controller extends NativeResource {
    ...
    Controller(long handle, long inputWidth) {
        super(handle, NativeLib::controllerDestroy);
    ...
    public Controller(SystemModel model, Formula spec, long inputWidth, long budgetNs, Bounds bounds,
        ...
        this(build(model, spec, inputWidth, budgetNs, bounds, smooth), inputWidth);
    }
    ...
    public double[] control(double[] state) throws SentilException {
        return NativeLib.controllerControl(handle(), state, inputWidth);
    }
}