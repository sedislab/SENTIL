...
HPC environments usually have no Docker. The same verification runs directly through the Makefile and cargo: `cargo test -p sentil` for the engine suite and `make -C sentil-ffi test-ffi` for the C ABI. The GPU tier runs inside an interactive allocation on a node with a device.