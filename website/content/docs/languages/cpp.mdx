...
Download the bundle for your platform and unpack it. Four are published: [`sentil-0.3.0-linux-x86_64.tar.gz`](https://github.com/sedislab/SENTIL/releases/download/v0.3.0/sentil-0.3.0-linux-x86_64.tar.gz), [`sentil-0.3.0-macos-x86_64.tar.gz`](https://github.com/sedislab/SENTIL/releases/download/v0.3.0/sentil-0.3.0-macos-x86_64.tar.gz), [`sentil-0.3.0-macos-arm64.tar.gz`](https://github.com/sedislab/SENTIL/releases/download/v0.3.0/sentil-0.3.0-macos-arm64.tar.gz), and [`sentil-0.3.0-windows-x86_64.tar.gz`](https://github.com/sedislab/SENTIL/releases/download/v0.3.0/sentil-0.3.0-windows-x86_64.tar.gz). Each unpacks into a prefix holding `include/sentil.h`, the libraries under `lib/`, `lib/pkgconfig/sentil.pc`, and `lib/cmake/Sentil/`. [`SHA256SUMS`](https://github.com/sedislab/SENTIL/releases/download/v0.3.0/SHA256SUMS) in the same release covers all four.

```bash
tar -xzf sentil-0.3.0-linux-x86_64.tar.gz
```

`tar` ships with Windows 10 and later, so the same line works from PowerShell. On Debian, Ubuntu, Fedora, and RHEL the C half can come from the distro packages in the same release, `libsentil-dev` or `libsentil-devel`, which the [C page](/docs/languages/c) covers.
