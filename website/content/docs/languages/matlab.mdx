...
Download [`Sentil.mltbx`](https://github.com/sedislab/SENTIL/releases/latest/download/Sentil.mltbx). The file carries no version in its name, so that link always fetches the newest release. To pin a version, open its release page, [v0.3.0](https://github.com/sedislab/SENTIL/releases/tag/v0.3.0) for the current build, and take the file from the assets there. The release workflow assembles it from a Linux, a Windows, and an Apple silicon build, so it is the same multi-platform package the File Exchange carries.
