...
% Breach baseline runner. Times Breach's dense-time STL robustness on the same
% oracle SENTIL runs and prints the shared JSON record, one per line. Breach is the
% dense-time robustness standard, so this is the full-signal dense comparison. Run as
% `matlab -batch "breach_runner('deterministic')"` with Breach on the path.
%
% If Breach's prebuilt mex files were compiled against a newer libstdc++ than the
% MATLAB in use, preload the system one first, for example
% `LD_PRELOAD=/usr/lib64/libstdc++.so.6 matlab -batch "breach_runner('deterministic')"`.
%
% Set BREACH_DIR to the Breach checkout if InitBreach is not already on the path.
