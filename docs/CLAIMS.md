...
UPPAAL-SMC runs it once the model is rebuilt for the fragment it samples, which `benchmarks/baselines/uppaal` carries. It estimates 0.0401 with a half-width of 0.004 in about 21 seconds, agreeing with the other three and running faster than either PRISM or Modest. Two faults had to go. The reference model raced the seven reactions with probability weights on ordinary edges, which UPPAAL ignores, selecting each edge with equal chance instead, so the rebuilt model races them through a branch point; a two-edge test with weights 9 and 1 returns a 1 to 1 split on ordinary edges and 9 to 1 through a branch point. It also carried a peak-detection sub-automaton whose general expression guards sit outside the samplable fragment, which is what produced the earlier degenerate verdict. UPPAAL picks its own run count from the requested half-width rather than taking a fixed sample size, and used 9462 runs here against the 10,000 of the others.
Command: `VERIFYTA=<verifyta> bash benchmarks/runners/uppaal_runner.sh benchmarks/baselines/uppaal/circadian.xml`, or `make bench-uppaal`.
