...
These are probabilistic model checkers, a different paradigm from SENTIL's trace and system monitoring, so a fair comparison needs a shared model every tool simulates. Two carry all four tools. The Barkai-Leibler circadian CTMC asks whether the activator reaches 100 within 20 time units, a rare-ish event near 0.04. The tandem queue asks whether either queue fills within 50 time units, a common event near 0.72, which exercises the opposite end of the variance range. Both are estimated at 10,000 samples, except by UPPAAL, which chooses its own count. Two further models, the biodiesel reactor and the powertrain controller, are continuous-state discrete-time recurrences, which PRISM cannot express; SENTIL, Modest, and UPPAAL all run them, and their agreement cross-checks the ports.

PRISM runs both cleanly: about 0.041 on the circadian model (0.0393 to 0.0438 across runs, half-width about 0.005) in about 27 seconds, and 0.726 on the tandem queue in about 5 seconds. SENTIL simulates the identical CTMCs with Gillespie's direct method and estimates the same probabilities by direct Monte Carlo, 0.0378 in about 1 second and 0.7151 in about 0.3 seconds, roughly 30 and 17 times faster with agreeing estimates. It also steps the two continuous-state Euler recurrences PRISM cannot express, 0.192 on the biodiesel reactor in about 10 seconds and 0.503 on the powertrain controller in under 2, an order of magnitude below the tens of seconds Modest and UPPAAL take on the same two models. Its `tandem.nm` needed one change to parse at all: the model named a constant `C`, which PRISM reserves for the cumulative reward operator.
...
Particle-count convergence. On a Gaussian random walk, the continuous-score regime the splitter is built for, the estimate tightens toward a Monte Carlo reference as the particle population grows. For a moderate event, truth 3.3e-2, the mean relative error over five seeds falls from 0.16 at 100 particles to 0.01 at 8000; for a rare event, truth 6.0e-5, which a thousand-sample plain run never sees, it falls from 0.38 to 0.07. The estimator folds the surviving fraction into the estimate at each level, so it stays consistent on a discrete score with heavy ties as well, not only on the continuous case.
...
Rare events against FIG, Modest, and PRISM. Two queueing models, both discrete-event, give a cross-tool check: a two-queue tandem where queue two overflows to capacity before it empties, `P(q2 > 0 U q2 == c)` at 5.602e-6, and a three-queue tandem where queue three does, at 1.274e-5. Four tools estimate the same probability on each. PRISM computes it exactly; FIG and Modest reach it by importance splitting, RESTART and fixed effort; SENTIL by adaptive multilevel splitting through the embedded jump chain. All four land on the exact value on both models, and SENTIL matches the two-queue value out to c=12 at 1.86e-8. Among the simulation methods SENTIL is the fastest by a wide margin, tens of milliseconds against Modest's and FIG's tens of seconds at comparable precision, since its embedded-chain step is a few arithmetic operations against their general model engines. PRISM's exact check is as quick here only because the state space is small; it stops being an option once the model is large or continuous, which is where the splitters are needed, and neither FIG nor Modest can express SENTIL's continuous-score Gaussian walk at all.
Command: `cargo run --release -p sentil-benchmarks --bin sentil_rare_tandem_runner` and `sentil_rare_3tandem_runner`; the baselines per `benchmarks/baselines/{fig,modest,prism}`.
Expected: the estimates match the exact overflow probability on both models within their variance, and SENTIL reaches a given precision faster than FIG and Modest. Tolerance: seed variance on the estimates; the speedups are machine and load dependent. Tier: CPU and tool-bound. Artifact: `benchmarks/results/rare_event_tandem.jsonl`, `rare_event_3tandem.jsonl`, `fig_rare_event.jsonl`, `modest_rare_event.jsonl`, `prism_rare_event.jsonl`.

A continuous-score second model, CPU and GPU. The tandem queue is discrete-event, so it runs on the CPU splitter only; the rare-event GPU path needs a continuous SimExpr model. An AR(1) crossing, `x_{t+1} = 0.9 x_t + noise` reaching a high level within the horizon, is that model. SENTIL estimates its 7.3e-4 crossing probability on both paths, each agreeing with a plain Monte Carlo reference, and the GPU fixed-effort splitter runs it about forty times faster than the CPU last-particle splitter at sixteen thousand particles, with the device time flat as the particle count grows while the CPU time scales with it. None of FIG, Modest, or PRISM can express a continuous real-valued walk at all, so this is a model only SENTIL covers among the four.
Command: `cargo run --release -p sentil-benchmarks --bin sentil_rare_walk_runner --features gpu` on a GPU node.
Expected: the CPU and GPU estimates agree with the reference, and the GPU is faster with flat scaling in the particle count. Tolerance: seed and scheme variance; the speedup is hardware dependent. Tier: GPU. Artifact: `benchmarks/results/rare_event_ou_walk.jsonl`.
