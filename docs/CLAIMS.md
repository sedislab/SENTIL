...
Rare events against FIG. The tandem-queue overflow, the benchmark FIG is built around, gives a cross-tool check on a discrete-event model. FIG, a dedicated rare-event simulator using RESTART importance splitting, and SENTIL's adaptive multilevel splitting estimate the same `P(q2 > 0 U q2 == c)`: queue two overflows to capacity before it empties. Both land on the exact overflow probability, 5.60e-6 at c=8 from solving the chain's linear system, and SENTIL matches it out to c=12 at 1.86e-8. At a matched 90 percent confidence and about ten percent relative precision SENTIL finishes in about 12 ms where FIG takes about a minute, roughly three to four thousand times faster, since its embedded-chain step is a few arithmetic operations against FIG's general IOSA engine. FIG cannot run the reverse comparison: its bounded-integer-and-clock state cannot express SENTIL's continuous-score Gaussian walk.
Command: `cargo run --release -p sentil-benchmarks --bin sentil_rare_tandem_runner`; FIG per `benchmarks/baselines/fig`.
Expected: the estimate matches the exact overflow probability within the splitting variance, and reaches a given precision faster than FIG. Tolerance: the estimate has seed variance; the speedup is machine and load dependent. Tier: CPU and tool-bound. Artifact: `benchmarks/results/rare_event_tandem.jsonl`, `fig_rare_event.jsonl`.
