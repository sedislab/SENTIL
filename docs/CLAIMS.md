...
Rare events against FIG, Modest, and PRISM. The tandem-queue overflow, the benchmark FIG is built around, is a cross-tool check on a discrete-event model: queue two overflows to capacity before it empties, `P(q2 > 0 U q2 == c)`. Three established tools estimate the same probability. PRISM computes it exactly, 5.602e-6 at c=8; FIG and Modest reach it by importance splitting, RESTART and fixed effort; SENTIL by adaptive multilevel splitting. All four land on the exact value, and SENTIL matches it out to c=12 at 1.86e-8. Among the simulation methods SENTIL is the fastest by a wide margin, about 12 ms against Modest's 18 seconds and FIG's tens of seconds at comparable precision, since its embedded-chain step is a few arithmetic operations against their general model engines. PRISM's exact check is as quick here only because the state space is small; it stops being an option once the model is large or continuous, which is where the splitters are needed, and neither FIG nor Modest can express SENTIL's continuous-score Gaussian walk at all.
Command: `cargo run --release -p sentil-benchmarks --bin sentil_rare_tandem_runner`; the baselines per `benchmarks/baselines/{fig,modest,prism}`.
Expected: the estimates match the exact overflow probability within their variance, and SENTIL reaches a given precision faster than FIG and Modest. Tolerance: seed variance on the estimates; the speedups are machine and load dependent. Tier: CPU and tool-bound. Artifact: `benchmarks/results/rare_event_tandem.jsonl`, `fig_rare_event.jsonl`, `modest_rare_event.jsonl`, `prism_rare_event.jsonl`.
